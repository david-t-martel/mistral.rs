...
pub fn get_engine_terminate_flag() -> Arc<AtomicBool> {
    ...
    let mut flags = ENGINE_TERMINATE_FLAGS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    flags
        .entry(thread_id)
...
impl Engine {
                ...
                SchedulerOutput::PagedAttention { mut output } => {
                    if !output.scheduled.is_empty() {
                        ...
                        let res = {
                            ...
                            let block_size = scheduler.block_size().expect("Block size should be available for paged attention");

                            let metadata = PagedAttentionMeta {
                                block_size,
                                sliding_window: pipeline.get_metadata().sliding_window,
                                block_engine: scheduler.block_engine().expect("Block engine should be available for paged attention"),
                            };
    ...
    fn replicate_request_to_daemons(&self, request: &Request) {
        if !distributed::is_daemon() && mistralrs_quant::distributed::use_nccl() {
            let name = distributed::ipc_name().expect("Failed to get IPC name for distributed communication");
            let num_workers =
                mistralrs_quant::distributed::get_global_tp_size_from_devices().expect("Failed to determine tensor parallel size") - 1;
            let listener = ListenerOptions::new().name(name).create_sync().expect("Failed to create IPC listener for distributed workers");

            for _ in 0..num_workers {
                let stream = listener.accept().expect("Failed to accept connection from distributed worker");
                let mut writer = BufWriter::new(stream);
                let req = format!("{}\n", serde_json::to_string(&request).expect("Failed to serialize request for distributed worker"));
                writer.write_all(req.as_bytes()).expect("Failed to write request to distributed worker");
            }
            ...
            for _ in 0..num_workers {
                let (stream, _) = listener.accept().expect("Failed to accept connection from distributed worker");
                let mut writer = BufWriter::new(stream);
                let req = format!("{}\n", serde_json::to_string(&request).expect("Failed to serialize request for distributed worker"));
                writer.write_all(req.as_bytes()).expect("Failed to write request to distributed worker");
            }
