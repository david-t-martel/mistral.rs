...
2. **Time-to-First-Token**: Latency measurement
3. **Concurrent Requests**: Load testing with multiple clients
4. **Flash Attention Impact**: Compare with/without feature

#### Priority 3: MCP Integration
1. Test each MCP server individually
2. Verify tool invocation from LLM responses
3. Validate error handling and timeouts
4. Document working vs broken servers
...
2. **Time MCP Deprecated**: Known broken server
...
4. **MCP Server Testing**: None validated yet
...
2. ✅ **COMPLETE**: Verify binary build with CUDA/Flash Attention
3. ⚠️ **PENDING**: Run model loading test with gemma2
4. ⚠️ **PENDING**: Measure inference performance
5. ⚠️ **PENDING**: Test MCP server integrations
...
2. Document performance benchmarks after manual testing

3. Create troubleshooting guide based on actual runtime issues
...
2. **test-mistralrs.ps1**: Automated test suite
   ```powershell
   .\test-mistralrs.ps1 -Quick          # Fast smoke test
   .\test-mistralrs.ps1 -Model gemma2   # Full test with model loading
   ```

3. **test-rag-redis.ps1**: MCP server validation
   ```powershell
   .\test-rag-redis.ps1
   ```

4. **launch-mistralrs.ps1**: Server launcher with MCP support
   ```powershell
